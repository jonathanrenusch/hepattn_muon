...
# Set matplotlib style to match individual task evaluators
...
class UnifiedTrackingEvaluator:
    ...
    def evaluate_task1_hit_track_assignment(self, region_type, output_dir):
        ...
        try:
            ...
            # Plot ROC curve with style matching individual script
    ...
    def evaluate_task2_track_validity(self, region_type, output_dir):
        ...
        try:
            ...
            # Plot ROC curve with style matching individual Task 2 script
            plt.figure(figsize=(8, 8))
            plt.plot(fpr, tpr, color='darkorange', lw=2, 
                    label=f'ROC curve (AUC = {roc_auc:.3f})')
            plt.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--', alpha=0.8)
    ...
    def _plot_efficiency_vs_pt(self, efficiency_results, output_dir, prefix, task_title):
        ...
        # Create step plots with error bands (matching individual task evaluator style)
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 10))
        
        # Get bin edges for step plotting (matching individual script style)
        pt_bins = np.linspace(0, 50, 21)  # 20 bins: 0 to 50 GeV linear (matching individual scripts)
        
        # Efficiency plot with step style and error bands (matching evaluate_task1_hit_track_assignment.py)
        for i, (lhs, rhs, eff_val, eff_err) in enumerate(zip(pt_bins[:-1], pt_bins[1:], efficiencies, eff_errors)):
            color = 'blue'
            
            # Create error band
            if eff_err > 0:
                ax1.fill_between([lhs, rhs], 
                               [max(0, eff_val - eff_err)] * 2,
                               [min(1, eff_val + eff_err)] * 2,
        ...
        ax1.set_ylabel(f'{task_title} Efficiency')
        ax1.set_ylim(0, 1.1)
        ax1.legend()
        ...
        for i, (lhs, rhs, fake_val, fake_err) in enumerate(zip(pt_bins[:-1], pt_bins[1:], fake_rates, fake_rate_errors)):
            color = 'red'
            
            # Create error band
            if fake_err > 0:
                ax2.fill_between([lhs, rhs], 
                               [max(0, fake_val - fake_err)] * 2,
                               [fake_val + fake_err] * 2,
                               alpha=0.3, color=color)
        ...
        ax2.set_ylabel(f'{task_title} Fake Rate')
        ax2.set_ylim(0, max(0.1, np.max(fake_rates) * 1.1) if len(fake_rates) > 0 else 0.1)
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        ax2.set_title(f'{task_title} Fake Rate vs $p_T$')
        
        # Set x-axis labels (matching individual scripts)
    ...
    def _plot_logit_distributions(self, logits, true_labels, output_dir, prefix, task_title):
        """Plot logit distributions for true vs false cases (matching individual Task 2 script style)."""
        ...
        plt.hist(false_logits, bins=50, alpha=0.7, label=f'Fake tracks (n={len(false_logits)})', 
                color='red', density=True)
        plt.hist(true_logits, bins=50, alpha=0.7, label=f'True tracks (n={len(true_logits)})', 
    ...
    def _plot_residual_distribution(self, residuals, output_dir, prefix, param_name):
        """Plot residual distribution as step histogram (matching individual Task 3 script style)."""
        plt.figure(figsize=(10, 6))
        
        # Check if residuals array is empty
        if len(residuals) == 0:
            plt.text(0.5, 0.5, 'No data available', transform=plt.gca().transAxes, 
                    ha='center', va='center', fontsize=14)
            plt.title(f'{param_name} Residual Distribution')
            plt.tight_layout()
        ...
        # Calculate truth-normalized residuals (matching evaluate_task3_regression.py)
        mean_val = np.mean(residuals)
        std_val = np.std(residuals)
        
        # Define bins for the histogram
        bins = np.linspace(np.percentile(residuals, 1), 
                          np.percentile(residuals, 99), 50)
        ...
        if len(bin_edges) <= 1:
            plt.text(0.5, 0.5, 'Insufficient data for histogram', transform=plt.gca().transAxes, 
                    ha='center', va='center', fontsize=14)
            plt.title(f'{param_name} Residual Distribution')
            plt.tight_layout()
            output_path = output_dir / f"{prefix}_distribution.png"
        ...
        # Create step histogram with thinner lines (matching individual script)
        plt.step(bin_edges[:-1], counts, where='post', linewidth=1, color='blue', 
                label=f'{param_name} Residuals')
        ...
        # Formatting
        plt.xlabel(f'{param_name} Residual (Pred - Truth)')
        plt.ylabel('Density')
        plt.title(f'{param_name} Residual Distribution')
        plt.grid(True, alpha=0.3)
        plt.legend()
        
        # Add text box with statistics (matching individual script)
    ...
    def _plot_overlaid_distributions(self, predictions, truth, output_dir, prefix, param_name):
        """Plot overlaid distributions of predictions vs truth (matching individual Task 3 script style)."""
        plt.figure(figsize=(10, 6))
        ...
        if len(predictions) == 0 or len(truth) == 0:
            plt.text(0.5, 0.5, 'No data available', transform=plt.gca().transAxes, 
                    ha='center', va='center', fontsize=14)
            plt.title(f'{param_name} Overlaid Distributions')
            plt.tight_layout()
            output_path = output_dir / f"{prefix}_overlaid.png"
            plt.savefig(output_path, dpi=150, bbox_inches='tight')
            plt.close()
            return
        
        # Calculate appropriate bins with more resolution (matching individual script)
        all_values = np.concatenate([predictions, truth])
        
        # Set axis ranges to match individual script
        if param_name == 'eta':
        ...
        # Plot both distributions with transparency (matching individual script)
        plt.hist(truth, bins=bins, alpha=0.6, density=True, 
                label='Ground Truth', color='blue', histtype='stepfilled')
        plt.hist(predictions, bins=bins, alpha=0.6, density=True, 
                label='Model Predictions', color='red', histtype='stepfilled')
        
        plt.xlabel(f'{param_name.capitalize()}')
        plt.ylabel('Density')
        plt.title(f'{param_name.capitalize()} Distribution: Predictions vs Ground Truth')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
    ...
    def _plot_residuals_vs_truth(self, residuals, truth, output_dir, prefix, param_name):
        """Plot residuals vs truth values (matching individual Task 3 script style)."""
        plt.figure(figsize=(10, 6))
        
        # Check if arrays are empty
        if len(residuals) == 0 or len(truth) == 0:
            plt.text(0.5, 0.5, 'No data available', transform=plt.gca().transAxes, 
                    ha='center', va='center', fontsize=14)
            plt.title(f'{param_name} Residuals vs Truth')
            plt.tight_layout()
            output_path = output_dir / f"{prefix}_vs_truth.png"
            plt.savefig(output_path, dpi=150, bbox_inches='tight')
            plt.close()
            return
        ...
        plt.xlabel(f'{param_name.capitalize()} Truth')
        plt.ylabel(f'{param_name.capitalize()} Residual (Predicted - Truth)')
        plt.title(f'{param_name.capitalize()} Residuals vs Truth')
        plt.grid(True, alpha=0.3)
