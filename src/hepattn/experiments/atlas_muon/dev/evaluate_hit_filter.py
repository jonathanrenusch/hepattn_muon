#!/usr/bin/env python3
"""
Comprehensive evaluation script for ATLAS muon hit filtering using DataLoader.
Generates ROC curves, efficiency/purity plots binned by truthMuon_pt.
"""
...
class AtlasMuonDatasetForEval(Dataset):
    """Minimal dataset implementation for evaluation purposes."""
    
    def __init__(self, dirpath, max_events=None):
        ...
        if max_events:
            self.file_indices = self.file_indices[:max_events]
            self.row_indices = self.row_indices[:max_events]
        
        self.num_events = len(self.file_indices)
        print(f"Dataset initialized with {self.num_events:,} events")
    
    def __len__(self):
        return self.num_events
    
    def __getitem__(self, idx):
        """Load minimal data needed for evaluation."""
        ...
        return {
            'event_idx': idx,
            'true_labels': torch.from_numpy(true_labels.astype(bool)),
            'hit_particle_ids': torch.from_numpy(hit_particle_ids),
            'hit_pts': torch.from_numpy(hit_pts.astype(np.float32)),
            'num_hits': num_hits,
            'num_tracks': num_tracks
        }

def collate_fn(batch):
    """Simple collate function for evaluation."""
    return batch  # Return list of dictionaries

class AtlasMuonEvaluator:
    """Evaluator for ATLAS muon hit filtering performance using DataLoader."""
    
    def __init__(self, eval_path, data_path, output_dir, max_events=None, num_workers=4):
        ...
        self.dataset = AtlasMuonDatasetForEval(
        ...
        self.dataloader = DataLoader(
            self.dataset,
            batch_size=1,  # Process one event at a time for simplicity
            shuffle=False,  # Keep sequential order for eval file matching
            num_workers=self.num_workers,
            collate_fn=collate_fn,
            pin_memory=True
        )
        
        print(f"Initialized evaluator with DataLoader")
        print(f"  Dataset size: {len(self.dataset)} events")
        print(f"  Using {self.num_workers} workers")
        print(f"  DataLoader batches: {len(self.dataloader)}")
    
    def collect_data(self):
        """Collect all data for analysis using DataLoader with multiple workers."""
        ...
        with h5py.File(self.eval_path, 'r') as eval_file:
            for batch in tqdm(self.dataloader, desc="Processing events"):
                # Since batch_size=1, we get a list with one dictionary
                data = batch[0]
                
                try:
                    event_idx = data['event_idx'].item()
                    
                    # Load predictions for this event
                    if str(event_idx) not in eval_file:
                        print(f"Warning: Event {event_idx} not found in eval file, skipping")
                        continue
                    ...
                    hit_particle_ids = data['hit_particle_ids'].numpy()
                    hit_pts = data['hit_pts'].numpy()
                    num_hits = data['num_hits'].item()
                    
                    # Verify shapes match
                    if len(hit_logits) != num_hits or len(hit_logits) != len(true_labels):
                        print(f"Warning: Shape mismatch in event {event_idx}, skipping")
                        continue
    ...
    def collect_data(self):
        ...
        with h5py.File(self.eval_path, 'r') as eval_file:
            for idx in tqdm(range(num_events), desc="Processing events"):
                try:
                    ...
                    hit_preds = eval_file[f"{idx}/preds/final/hit_filter/hit_on_valid_particle"][0]
                    
                    # Verify shapes match
                    if len(hit_logits) != num_hits:
                        print(f"Warning: Shape mismatch in event {idx}, skipping")
                        continue
                    
                    # Get truth labels
                    true_labels = hits["on_valid_particle"]
                    
                    # Map hits to particles and get pt values
                    hit_particle_ids = hits["spacePoint_truthLink"]
                    
                    # For each hit, find the corresponding particle pt
                    hit_pts = np.full(len(hit_logits), -1.0)  # Default for noise hits
                    
                    for i, particle_id in enumerate(hit_particle_ids):
                        if particle_id >= 0:  # Valid particle
                            # Find this particle in the particles array
                            particle_mask = particles['particle_id'] == particle_id
                            if np.any(particle_mask):
                                pt_idx = np.where(particle_mask)[0][0]
                                if pt_idx < len(particles['truthMuon_pt']):
                                    hit_pts[i] = particles['truthMuon_pt'][pt_idx]
        ...
        # Debug: Check pt distribution
        valid_pts = self.all_particle_pts[self.all_particle_pts > 0]
        if len(valid_pts) > 0:
            print(f"PT statistics for valid particles:")
            print(f"  Min: {np.min(valid_pts):.1f} GeV")
            print(f"  Max: {np.max(valid_pts):.1f} GeV") 
            print(f"  Mean: {np.mean(valid_pts):.1f} GeV")
            print(f"  Median: {np.median(valid_pts):.1f} GeV")
        
        # Debug: Check truth label distribution for different pt ranges
        true_hits_with_valid_pt = np.sum((self.all_true_labels) & (self.all_particle_pts > 0))
        print(f"True hits with valid pt: {true_hits_with_valid_pt}")
        print(f"True hits with invalid pt (noise): {np.sum(self.all_true_labels) - true_hits_with_valid_pt}")
    ...
    def calculate_efficiency_purity_by_pt(self, working_point):
        ...
        # working_point is now the target efficiency/recall
        ...
        if not np.any(valid_indices):
            ...
            threshold = thresholds[-1]  # Use the most lenient threshold
        else:
            threshold = thresholds[valid_indices][0]  # First threshold that achieves target efficiency
        ...
        # First, let's check the overall performance to debug
        overall_tp = np.sum(predictions & self.all_true_labels)
        overall_fn = np.sum(~predictions & self.all_true_labels)
        overall_efficiency = overall_tp / (overall_tp + overall_fn) if (overall_tp + overall_fn) > 0 else 0
        print(f"Overall efficiency achieved: {overall_efficiency:.3f}")
        
        # Only consider hits that belong to valid particles (pt > 0)
        valid_particle_mask = self.all_particle_pts > 0
        
        # Find pt range for valid particles only
        if np.any(valid_particle_mask):
            pt_min = np.min(self.all_particle_pts[valid_particle_mask])
            pt_max = np.max(self.all_particle_pts[valid_particle_mask])
            print(f"PT range for valid particles: {pt_min:.1f} to {pt_max:.1f} GeV")
        else:
            print("Warning: No valid particles found!")
            pt_min, pt_max = 0, 100
        
        # Define pt bins (20 bins from pt_min to pt_max, but ensure reasonable range)
        pt_min = max(0, pt_min)
        pt_max = min(200, max(pt_max, 50))  # Ensure at least 50 GeV range
        pt_bins = np.linspace(pt_min, pt_max, 21)
        ...
        for i in range(len(pt_bins) - 1):
            # Get hits in this pt bin - only for hits that belong to valid particles
            # and are true muon hits
            pt_mask = ((self.all_particle_pts >= pt_bins[i]) & 
                      (self.all_particle_pts < pt_bins[i+1]) & 
                      (self.all_particle_pts > 0) &  # Must be valid particle
                      self.all_true_labels)  # Must be true hits
            ...
            # Calculate efficiency (recall) - fraction of true hits that are predicted as true
            ...
            # Calculate purity (precision) - fraction of predicted hits that are actually true
            # For purity, we need all predictions in this pt range (including noise)
            pt_mask_all = ((self.all_particle_pts >= pt_bins[i]) & 
                          (self.all_particle_pts < pt_bins[i+1]))
            
            # Also include noise hits in this spatial/detector region for purity calculation
            # Since we don't have spatial info easily, let's use a simpler approach
            # Focus on hits that either belong to particles in this pt range OR are noise
            ...
            # Group hits by particle and count tracks with < 3 hits
            unique_particles = np.unique(self.all_particle_ids[pt_mask & predictions])
            tracks_lost = 0
            total_tracks = 0
            
            for particle_id in np.unique(self.all_particle_ids[pt_mask]):
            ...
            # Calculate uncertainties (using binomial statistics)
    ...
    def plot_efficiency_purity_by_pt(self):
        """Plot efficiency and purity vs pt for all working points."""
        ...
        for wp in self.working_points:
            ...
            (pt_centers, efficiencies, purities, 
             eff_errors, pur_errors, tracks_lost) = self.calculate_efficiency_purity_by_pt(wp)
            
            # Calculate average metrics (weighted by number of hits in each bin)
            ...
            # Plot with filled error bands (similar to the attached plot)
            ...
            plt.errorbar(pt_centers, efficiencies, yerr=eff_errors, 
                        marker='o', capsize=4, capthick=2, markersize=6,
            ...
            # Add text box with statistics (styled like the attached plot)
            ...
            plt.errorbar(pt_centers, purities, yerr=pur_errors, 
                        marker='s', capsize=4, capthick=2, markersize=6,
                        color=color, linewidth=2, linestyle='-',
    ...
    def generate_summary_report(self):
        ...
        # ROC AUC
        fpr, tpr, _ = roc_curve(self.all_true_labels, self.all_logits)
        roc_auc = auc(fpr, tpr)
        ...
        # Calculate ROC curve once for threshold determination
        fpr, tpr, thresholds = roc_curve(self.all_true_labels, self.all_logits)
