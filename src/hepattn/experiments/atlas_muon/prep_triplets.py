                ...
                for event_idx_in_chunk in range(chunk_end - chunk_start):
                    ...
                    for track_idx in valid_tracks:
                        if track_features_chunk['truthMuon_pt'][event_idx_in_chunk][track_idx] < self.pt_threshold or \
                                abs(track_features_chunk['truthMuon_eta'][event_idx_in_chunk][track_idx]) > self.eta_threshold or \
                                    np.sum(hit_features_chunk['spacePoint_truthLink'][event_idx_in_chunk] == track_idx) < self.num_hits_threshold:
                    ...
                    for track_idx in remaining_tracks:
                        # finding unique station phi, eta and index
                        track_mask = hit_features_chunk['spacePoint_truthLink'][event_idx_in_chunk] == track_idx
                        unique_station_phi = np.unique(hit_features_chunk['spacePoint_stationPhi'][event_idx_in_chunk][track_mask])
                        unique_station_eta = np.unique(hit_features_chunk['spacePoint_stationEta'][event_idx_in_chunk][track_mask])
                        unique_station_indices = np.unique(hit_features_chunk['spacePoint_stationIndex'][event_idx_in_chunk][track_mask])
