...
class Task3RegressionEvaluator:
    ...
    def plot_charge_accuracy_vs_variable(self, data, variable='pt', output_subdir=None, category_name=""):
        ...
        for i, (lhs, rhs, acc_val, acc_err) in enumerate(zip(bins[:-1], bins[1:], accuracies, acc_errors)):
            color = 'green'
            ...
            if acc_err > 0:
                ...
                ax.fill_between(point_in_range, y_upper, y_lower, 
                               ...
                               label="binomial err - Accuracy" if i == 0 else "")
            
            # Step plot
            ax.step([lhs, rhs], [acc_val, acc_val], 
                   color=color, linewidth=2.5,
                   label="Charge Classification Accuracy" if i == 0 else "")
    ...
    def plot_residuals(self, data, param, output_dir, category_name, statistics):
        ...
        self._plot_single_residual_type(absolute_residuals, param, output_dir, category_name,
                                      "absolute", abs_mean, abs_std)
        
    def _plot_single_residual_type(self, residuals, param, output_dir, category_name, 
                                 residual_type, mean_val, std_val):
        ...
        plt.step(x_step, y_step, where='post', linewidth=1, color='blue',
                 label=f'{param.capitalize()} {residual_type.capitalize()} Residuals')
        ...
        plt.xlabel(xlabel)
        plt.title(f'{param.capitalize()} {title_suffix} Residual Distribution - {category_name}\n(Centered on mean ± 3σ, outliers clipped to edge bins)')
        plt.ylabel('Count')
        ...
        print(f"Saved {category_name} {param} {residual_type} residual plot to {output_path} (mean ± 3σ range, {n_clipped} outliers clipped)")
